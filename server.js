...
// Endpoint untuk generate poster produk
app.post('/generate/poster', async (req, res) => {
  ...
  try {
    ...
    if (result.status === 'succeeded') {
      ...
      res.status(500).json({ error: 'Gagal generate poster' });
    }
  } catch (err) {
...
// Endpoint untuk generate karikatur dari foto wajah
app.post('/generate/karikatur', upload.single('image'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'No image' });

  const style =
    req.body.style === 'anime' ? 'anime style'
    : req.body.style === 'sketch' ? 'pencil sketch'
    : 'cartoon style';
